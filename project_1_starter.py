...
def get_top_n(prev_returns, top_n):
    ...
    for index,row in top_stocks.iterrows():
        t=row.nlargest(top_n).index
        
        top_stocks.loc[index]=0
        top_stocks.loc[index,t]=1
    
    top_stocks=top_stocks.fillna(0).astype(np.int)
    return top_stocks
...
def portfolio_returns(df_long, df_short, lookahead_returns, n_stocks):
    ...
    df_short_weight = -1 * (df_short/n_stocks)
    return (df_long_weights + df_short_weight)*lookahead_returns
...
def analyze_alpha(expected_portfolio_returns_by_date):
    ...
    # TODO: Implement Function
    nh=0
    t, p = stats.ttest_1samp(expected_portfolio_returns_by_date, nh)
    return (t,p/2)
...
# 0.07, which is more than 0.05 hence we do not have evidence to reject the null hypothesis so we cannot say the porfolio is profitable.
